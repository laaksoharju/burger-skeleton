...
<div id="ordering" class=container>
  ...
  <div id="huvudmeny">
    ...
    <button id="btn1" class="btn active" v-on:click="highlightButton(); redirect1()">{{uiLabels.sides}}</button>
    <button id="btn2" class="btn" v-on:click="highlightButton() ; redirect2()">{{uiLabels.drinks}}</button>
  </div>
...
export default {
  ...
  methods: {
    ...
    previousCategory: function() {
      if (this.category > 5 && this.category-1 >4){
      this.category -= 1;
      var btns = document.getElementsByClassName("btn");
